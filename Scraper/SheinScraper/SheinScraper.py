...
class SheinScraper(AbstractScraper):
    ...
    async def _get_celeb_fashion_data_async(self, session: aiohttp.ClientSession, product: str,
                                           colors: List[str], gender: str) -> dict:
    ...
    async def _scrape_product_data_async(self, session: aiohttp.ClientSession, product: str,
                                         colors: List[str], gender: str, item_key: str):
        product = product.strip()
        product_to_search = product + " " + item_key
        response_data = await self._get_celeb_fashion_data_async(session, product_to_search, colors, gender)
        product_data = []
        ...
        for i in range(len(response_data['goods'])):
            cur_result = response_data['goods'][i]
            # Extract relevant data
            name = cur_result['goods_url_name']
            url = f"https://il.shein.com/{name}-p-{cur_result['goods_id']}-cat-{cur_result['cat_id']}.html?" \
                  f"src_identifier={self._params['src_identifier']}&mallCode=1"
            image_url = cur_result['goods_img']
            cur_data = {
                            'name': name,
                            'url': url,
            ...
            product_data.append(cur_data)
            # product_data.append(ProductData(name=name, url=url, imageUrl=image_url))

        return product_data

    async def scrape_celeb_fashion_data_async(self, ai_json_like_data: AIJsonLikeData) -> dict[str, str]:
            ...
            for item_key, value in ai_json_like_data.items():
                if item_key != 'name' and item_key != 'gender' and item_key != 'colors' and item_key != 'conclusion':
                    ...
                    for product in products:
                        product = product.strip()
                        tasks.append(self._scrape_product_data_async(session, product, colors, gender, item_key))
    ...
    #
    # def scrape_celeb_fashion_data(self, ai_json_like_data: AIJsonLikeData):
    #     scraped_data = await self.scrape_celeb_fashion_data(ai_json_like_data)
    #     return scraped_data
