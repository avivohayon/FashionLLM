...
class SheinScraper(AbstractScraper):
    ...
    def get_product_data(self, product:str , item_key:str, colors: list[str], gender:str, celebrity_data_dict: dict):
        """
    ...
    def scrape_celeb_fashion_data(self, ai_json_like_data: AIJsonLikeData) -> dict[str:str]:
        print("multi thread celeb_fashion_data start")
        start = perf_counter()
        ...
        for item_key, value in ai_json_like_data.items():
            if item_key == 'conclusion':
                ...
                # copy the working dict value filled with the threads tasks output
                celebrity_data_dict[item_key] = working_dict[item_key]
